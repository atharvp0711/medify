...
export default function DaySelector({
  ...
  setSelectedDate = () => {}, // Default to a no-op function
  totalSlots = 0, // Default to 0 slots
}) {
  ...
  const handleClick = (day) => {
    if (typeof setSelectedDate === "function") {
      ...
      console.error(
        "setSelectedDate is not a function. Please pass a valid function."
      );
    }
  };
        ...
        {dateItems.map((day, index) => (
          <SwiperSlide key={index} className={styles.swiperslide}>
            <Stack
              ...
              sx={{ cursor: "pointer" }}
              role="button"
              aria-pressed={isEqual(day, selectedDate)}
              aria-label={`Select date: ${format(day, "PPPP")}`}
            >
              <Typography
